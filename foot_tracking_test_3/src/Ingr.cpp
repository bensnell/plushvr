...
// -----------------------------------------------------------
void Ingr::setup(string _name, int _nValues, float inLowerBound, float inUpperBound) {
...
void Ingr::difference() {
    ...
    cook = differenceCalc(cook, &prevDiff, &bPrevDiff);
}

// -----------------------------------------------------------
float Ingr::differenceCalc(float _thisValue, float *_prevValue, bool *_bPrevValue) {
    
    float result;
    
    if (*_bPrevValue) {
        // previous data exists
        result = _thisValue - *_prevValue;
        ...
        for (int j = 0; j < steps.size(); j++) {
            switch(steps[j]) {
                ...
                case INV:
                    ...
                    break;
                case DIFF:
                    tmpCook = differenceCalc(tmpCook, &_prevDiff, &_bPrevDiff);
